...
*   **子标题**: 正文中以 `#` 开头的行将被解析为子标题（加粗，增加间距）。
    *   示例: `## 科技动态`
*   **正文**: 普通文本行。
*   **分隔线**: 只有标题下方会自动绘制分隔线，正文中不支持 Markdown 分隔线。

### 3. 推荐 Prompt 示例
> 请为我生成一份简报，格式要求如下：
> 1. 第一行必须是 `# ` 开头的主标题，不超过 10 个字。
> 2. 正文部分可以包含 `##` 开头的子标题（如 `##早安语录`）。
> 3. 每行正文不要超过 18 个汉字，超过请换行。
> 4. 总行数控制在 12 行以内，不要使用 Markdown 列表符号（- 或 1.），直接写内容。
