...
### 1. 总体限制
*   **分辨率**: 单侧屏幕有效显示区域约为 360x380 像素。
*   **自动换行**: 正文和标题均支持根据屏幕宽度自动换行，无需手动控制行长。
*   **字数**: 单屏建议总字数控制在 **250 字** 以内，以免内容超出屏幕底部。

### 2. 格式语法
*   **大标题**: 第一行如果以 `# ` 开头，将被解析为主标题（加粗，字号大）。
    *   示例: `# 每日新闻简报`
*   **子标题**: 正文中以 `#` 开头的行将被解析为子标题（加粗，增加间距）。
    *   示例: `## 科技动态` (注：程序兼容 `#` 和 `##` 开头，推荐使用标准 Markdown 二级标题)
*   **正文**: 普通文本行。
*   **分隔线**: 自动绘制，无需手动添加。

### 3. 推荐 Prompt 示例
> 请为我生成一份简报，格式要求如下：
> 1. 第一行必须是 `# ` 开头的主标题，不超过 10 个字。
> 2. 正文部分可以包含 `##` 开头的子标题（如 `## 当地天气`）。
> 3. 正文内容无需手动换行，程序会自动排版。
> 4. 单屏总字数控制在 250 字以内，不要使用 Markdown 列表符号（- 或 1.），直接写内容。
